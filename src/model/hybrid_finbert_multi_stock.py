...
# Load the CSV
df = pd.read_csv("../../data/merged_finbert_market_data.csv", index_col="Date", parse_dates=True)
...
# 1. Baseline Memory (EWMA)
df_clean["finbert_ewma_5"] = df_clean.groupby('ticker')["finbert_score"].transform(
    lambda x: x.ewm(span=5, min_periods=1).mean()
)

# 2. Volume-Weighted Impact
df_clean["sentiment_impact"] = df_clean["finbert_score"] * np.log1p(df_clean["news_count"])

# 3. The "Surprise" Factor (Delta between Today and the 5-Day Trend)
df_clean["sentiment_delta"] = df_clean["finbert_score"] - df_clean["finbert_ewma_5"]

# 4. The "Frenzy" Factor (News Volume Spike vs 20-Day Average)
# We add +1 to the denominator to prevent division by zero on dead weeks
df_clean["news_volume_spike"] = df_clean["news_count"] / (
    df_clean.groupby('ticker')["news_count"].transform(
        lambda x: x.rolling(window=20, min_periods=1).mean()
    ) + 1
)

# 5. The "Chaos" Factor (5-Day Rolling Volatility of Sentiment)
df_clean["sentiment_volatility"] = df_clean.groupby('ticker')["finbert_score"].transform(
    lambda x: x.rolling(window=5, min_periods=1).std()
)

# Fill any residual NaNs created by rolling windows/std devs
...
# feature_cols.extend(finbert_cols)
...
X_train_3D, y_train_final, today_news_train = create_sequences_multi(train_df, feature_cols, finbert_cols, lookback_days, ticker_cols)
X_val_3D, y_val_final, today_news_val = create_sequences_multi(val_df, feature_cols, finbert_cols, lookback_days, ticker_cols)
X_test_3D, y_test_final, today_news_test = create_sequences_multi(test_df, feature_cols, finbert_cols, lookback_days, ticker_cols)
...
print(f"Today Features Shape: {today_tech_train.shape}")
...
plotting_helper.plot_xgboost_train_graph(xgb_model.evals_result(), "hybrid_finbert")
plotting_helper.plot_feature_importance(xgb_model, feature_cols, finbert_cols, "hybrid_finbert")
...
upper_threshold = np.percentile(y_pred, 70)  # Top 25% most bullish predictions
...
for i, ticker_col in enumerate(ticker_cols):
    ...
    X_train_3D_stock, y_train_actual, today_news_train_stock = create_sequences_multi(stock_train_df, feature_cols, finbert_cols, lookback_days, [ticker_col])
    X_val_3D_stock, y_val_actual, today_news_val_stock = create_sequences_multi(stock_val_df, feature_cols, finbert_cols, lookback_days, [ticker_col])
    X_test_3D_stock, y_test_actual, today_news_test_stock = create_sequences_multi(stock_test_df, feature_cols, finbert_cols, lookback_days, [ticker_col])
...
# Show the plot
file_name = f"../../output/hybrid_finbert_multi_prediction_{datetime.now().strftime('%Y%m%dT%H%M%S')}.png"
